...
const updateHargaBarang = (index) => {
    ...
    form.value.details[index].harga_beli = selected ? selected.harga_beli : 0;
};
...
const submitForm = () => {
    ...
    router.post(route('purchase-order.store'), form.value, {
        onSuccess: () => {
            router.push(route('purchase-order'));
            alert('Purchase Order & Detail berhasil disimpan.');
        },
        onError: (error) => {
            console.error(error);
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Create Purchase Order</h2>
        </template>
                    ...
                    <form @submit.prevent="submitForm">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select v-model="form.supplier_id" class="w-full p-2 border rounded" required>
                                    <option value="" disabled selected>Pilih</option>
                                    <option v-for="s in props.supplier" :value="s.id" :key="s.id">
                                        {{ s.nama }}
                                    </option>
                                </select>
                            </div>
                            ...
                            <div v-for="(item, index) in form.details" :key="index" class="border-b py-4 mb-4">
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 details-end">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" v-model="item.jumlah" min="1" class="w-full p-2 border rounded" required />
                                    </div>
                                    <div>
                                        <label class="text-sm">Harga Beli</label>
                                        <input type="number" :value="item.harga_beli" readonly class="w-full p-2 border rounded bg-gray-100" />
                                    </div>
                                    <div class="mt-2">
