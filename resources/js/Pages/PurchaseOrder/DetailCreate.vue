...
const updateHargaBarang = (index) => {
  ...
  form.items[index].harga_beli = selected ? selected.harga_beli : 0;
};

const submitForm = () => {
  form.post(route('purchase-order-detail.store-multiple', props.purchaseOrder.id), {
    onSuccess: () => {
      alert('Semua detail purchase order berhasil ditambahkan');
    },
    onError: (error) => {
      console.error(error);
    },
...
<template>
  <div>
    <Head title="Tambah Detail Purchase Order" />
    <AuthenticatedLayout>
      <template #header>
        <div class="flex justify-between">
          <h2 class="text-xl font-semibold leading-tight text-gray-800">
            Tambah Detail Purchase Order
          </h2>
        </div>
      </template>
