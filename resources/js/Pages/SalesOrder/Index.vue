...
function updateStatusToCompleted(id) {
    if (confirm('Apakah Anda yakin ingin mengubah status menjadi selesai?')) {
        Inertia.put(route('sales-order.update-status', id));
    }
}
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between">
                ...
                <Link :href="route('sales-order.create')" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                    Add Sales Order
                </Link>
            </div>
        </template>
                                ...
                                <tr v-for="item in sortedSalesOrder" :key="item.id">
                                    ...
                                    <td class="py-2 px-4 border">
                                        <div class="flex gap-2">
                                            ...
                                            <a :href="route('sales-order.pdf', item.id)" target="_blank" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">
